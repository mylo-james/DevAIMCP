...
describe('Policy engine (unit, mocked DB)', () => {
	...
	it('denies actions listed in deny_actions', async () => {
		vi.spyOn(db, 'query').mockResolvedValue({ rows: [{ name: 'deny', rules: { deny_actions: ['delete_project'] }, active: true }] } as any);
	...
	it('allows actions when not denied and agent allowed', async () => {
		vi.spyOn(db, 'query').mockResolvedValue({ rows: [{ name: 'allow-dev', rules: { allow_agents: ['dev'] }, active: true }] } as any);
	...
	it('requires context when context_required is true', async () => {
		vi.spyOn(db, 'query').mockResolvedValue({ rows: [{ name: 'require-context', rules: { context_required: true }, active: true }] } as any);
