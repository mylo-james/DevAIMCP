...
export async function createProject(input: Omit<Project, 'id' | 'created_at' | 'updated_at'>): Promise<Project> {
	...
	const values = [input.name, input.description ?? null, input.repository_url ?? null, input.language ?? null, input.framework ?? null, input.metadata ?? null];
...
export async function storeMemory(input: Omit<Memory, 'id' | 'created_at' | 'updated_at'>): Promise<Memory> {
...
export async function createStory(input: Omit<Story, 'id' | 'created_at' | 'updated_at'>): Promise<Story> {
...
export async function updateStory(id: number, patch: Partial<Omit<Story, 'id' | 'created_at' | 'updated_at'>>): Promise<Story | null> {
...
export async function listStories(projectId: number): Promise<Story[]> {
	const { rows } = await query<Story>('SELECT * FROM stories WHERE project_id = $1 ORDER BY id ASC', [projectId]);
	return rows;
}
