...
export async function runBmadWebBuilder(args: {
  ...
  const modPath = path.resolve(process.cwd(), 'seed/vendor/bmad-tools/builders/web-builder.js');
...
export async function runBmadYamlFormat(args: { filePath: string }): Promise<RunResult> {
  const modPath = path.resolve(process.cwd(), 'seed/vendor/bmad-tools/yaml-format.js');
  const mod = await import(modPath);
...
export async function runBmadFlatten(args: {
  ...
  const aggPath = path.resolve(process.cwd(), 'seed/vendor/bmad-tools/flattener/aggregate.js');
  const mod = await import(aggPath);
  const flatten = (mod.flatten || mod.default) as (i: string, o: string) => Promise<any>;
...
export async function runBmadVersionBump(args: {
  ...
  const modPath = path.resolve(process.cwd(), 'seed/vendor/bmad-tools/version-bump.js');
  try {
    const mod = await import(modPath);
    const bump = (mod.bumpVersion || mod.default) as (t?: string) => Promise<any>;
...
export async function runBmadResolveDeps(args: { agentOrTeam: string }): Promise<RunResult> {
  const resolverPath = path.resolve(
    process.cwd(),
    'seed/vendor/bmad-tools/lib/dependency-resolver.js'
  );
  try {
    const Res = (await import(resolverPath)).default || (await import(resolverPath));
    const root = path.resolve(process.cwd(), 'docs/devai-method');
    const resolver = new (Res as any)(root);
    let res: any;
    if (args.agentOrTeam.endsWith('.yaml')) {
      res = await resolver.resolveTeamDependencies(args.agentOrTeam.replace(/\.yaml$/, ''));
    } else {
      ...
      `deps: ${JSON.stringify(
        res && {
          ...
          counts: {
            ...
            agents: Array.isArray(res.agents) ? res.agents.length : res.agent ? 1 : 0,
          },
        }
      )}`
    );
