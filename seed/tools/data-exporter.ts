  ...
  switch (action) {
    ...
    case 'memories': {
      ...
      const res = await query(
        'SELECT id, project_id, memory_type, content, context, reasoning, confidence, tags, created_at FROM memories WHERE project_id = $1 ORDER BY id ASC',
        [id]
      );
      const rows = res.rows;
      if (format === 'json')
      ...
      if (format === 'html')
        return {
          content: [
            { type: 'text', text: wrapHtml(`<h1>Memories</h1><pre>${toMarkdown(rows)}</pre>`) },
          ],
        };
      return { content: [{ type: 'text', text: JSON.stringify(rows, null, 2) }] };
    }
