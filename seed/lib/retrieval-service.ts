...
export class RetrievalService {
  ...
  private static async applyACLFilter(
    ...
    for (const result of results) {
      const accessCheck = await AuthorizationService.checkResourceAccess(
        actorId,
        result.resource.id
      );
  ...
  private static async applyGlobalRecency(
    ...
    return results.map(result => ({
      ...result,
      combinedScore:
        result.vectorScore + result.actorImportance * 0.1 + result.globalRecency * 0.15,
    }));
  }
   ...
   */
  static async searchWithFeedback(
    params: RetrievalParams & {
  ...
  static async getRetrievalStats(actorId: number): Promise<{
    ...
    return {
      ...
      avgResultsPerQuery:
        searchActivities.length > 0 ? importanceIncrements.length / searchActivities.length : 0,
  ...
  static async advancedSearch(
    ...
    switch (params.rankingStrategy) {
      ...
      case 'recency_boosted':
        return baseResults.sort(
          (a, b) => b.vectorScore + b.globalRecency * 0.3 - (a.vectorScore + a.globalRecency * 0.3)
        );
